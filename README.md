...
- **`2D/`** — S¹ × ℝ. The world is a circle crossed with a height line.
- **`3D/`** — S² × ℝ. The world is a sphere crossed with a height line.
- **`mobius/`** — A Möbius strip world. Walking around once turns you upside-down relative to where you started.

Each is a single HTML file. Open it in a browser; no build step.
